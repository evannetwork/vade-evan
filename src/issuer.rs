...
pub mod issuer {
  ...
  pub struct Issuer {
    public_key_registry: HashMap<String, CredentialPublicKey>,
    schema_registry: HashMap<String, CryptoCredentialSchema>,
    credential_def_registry: HashMap<String, CredentialDefinition>
  }
  ...
  impl Issuer {
    ...
    pub fn new() -> Issuer {
      Issuer {
          ...
          credential_def_registry: HashMap::new()
      }
    }

    fn create_credential_definition(
      definition_issuer: String,
      credential_schema: CredentialSchema,
      include_master_secret: bool,
      issuer_private_key: String
    ) -> (CredentialPrivateKey, CredentialDefinition) {
      ...
      let credential_schema_builder = CryptoIssuer::new_credential_schema_builder().unwrap();
    ...
    pub fn sign_credential_with_revocation(
      ...
      credential_private_key: CredentialPrivateKey,
      credential_public_key: CredentialPublicKey,
      credential_revocation_definition: RevocationRegistryDefinition
      credential_revocation_id: u32,

    ) -> SignedCredential {
      let credential_issuance_nonce = new_nonce().unwrap();

      let (cred, proof) = CryptoIssuer::sign_credential_with_revoc(rev_idx: u32, max_cred_num: u32, issuance_by_default: bool, rev_reg: &mut RevocationRegistry, rev_key_priv: &RevocationKeyPrivate, rev_tails_accessor: &RTA)(&credential_request.subject,
                                &credential_request.blinded_credential_secrets,
                                &credential_request.blinded_credential_secrets_correctness_proof,
                                &credential_request.credential_nonce,
                                &credential_issuance_nonce,
